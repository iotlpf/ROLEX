...
public:
  ...
  FastRandom() : seed(0) { set_seed0(seed); }
  ...
  template <typename T = i64> inline i64 rand_number(T min, T max) {
    return check_between_inclusive(
        (i64)(next_uniform() * (max - min + 1) + min), min, max);
  }
